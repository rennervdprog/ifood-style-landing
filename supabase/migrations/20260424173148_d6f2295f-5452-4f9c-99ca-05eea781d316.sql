...
-- Atualizar lojas existentes no plano comissão de 5% para 6%
UPDATE public.store_plans
SET commission_rate = 6
WHERE plan_type = 'commission_only' AND is_active = true AND commission_rate = 5;

-- Atualizar default de commission_rate na tabela stores
ALTER TABLE public.stores ALTER COLUMN commission_rate SET DEFAULT 6;

-- Atualizar lojas com commission_rate antigo (5%) que ainda usam o fallback
UPDATE public.stores SET commission_rate = 6 WHERE commission_rate = 5;

-- Atualizar default na função get_store_commission_rate (fallback)
...
-- Atualizar register_as_lojista para usar 6% no plano comissão
...
-- Atualizar handle_new_user (trigger de cadastro) para usar 6%
