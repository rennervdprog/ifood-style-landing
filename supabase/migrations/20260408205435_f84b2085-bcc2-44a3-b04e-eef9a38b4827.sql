...
-- Update validate_order_prices: always 15% commission
...
AS $function$
BEGIN
  -- Commission is always 15% of subtotal regardless of delivery mode
  NEW.app_fee := ROUND(COALESCE(NEW.subtotal, 0) * 0.15, 2);
...
-- Update verify_order_subtotal: always 15%
...
-- Update client_confirm_delivery: always 15%
...
-- Update driver_confirm_store_return: always 15%
...
-- Update auto_finalize_stale_orders: always 15%
...
BEGIN
  ...
  -- Generate 15% commission for auto-finalized orders
  INSERT INTO store_balances (store_id, comissao_pendente, pending_commission, repasse_pendente, updated_at)
  SELECT 
    o.store_id,
