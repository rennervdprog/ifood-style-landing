-- Documentos legais v6.0: alinhamento entre oferta digital, LGPD e serviços implementados.
-- A inserção de uma nova versão atual aciona o reaceite já consumido no web e no app nativo.
...
WITH new_terms AS (
  INSERT INTO public.legal_documents (
    ...
    $terms_v6$# Termos de Uso do ItaSuper

**Versão 6.0 — vigente a partir de 17 de agosto de 2026**
...
## 6. Pagamentos, valores devidos e cobranças de Lojistas
...
Nos pagamentos físicos ou em modalidades em que o Lojista recebe diretamente do Cliente, eventuais valores devidos pelo Lojista ao ItaSuper são apurados conforme seu plano, contrato comercial, painel financeiro e regras exibidas antes da contratação ou cobrança. Percentuais, mensalidades, taxas, componentes de entrega, ciclos de cobrança, limites e vencimentos podem variar por plano, data de adesão, acordo comercial e configuração da loja. A cobrança manual ou automática deverá identificar o valor, a origem e o meio de regularização disponibilizado.

O Lojista deve conferir os lançamentos exibidos no painel e comunicar divergências pelo suporte antes do vencimento, sempre que possível. Em caso de inadimplência, o ItaSuper poderá enviar avisos, restringir funcionalidades não essenciais ou suspender o acesso conforme a gravidade, o contrato comercial e a legislação, assegurando canal de atendimento para regularização e preservando o tratamento adequado de pedidos em andamento quando aplicável.
...
$terms_v6$,
    'Regras gerais atualizadas para marketplace, pedidos, pagamentos, cancelamentos, entregas, comunicações, IA e tratamento de dados; valores comerciais passam a seguir o painel, contrato ou checkout aplicável.',
    true
  )
  RETURNING id
),
new_privacy AS (
  INSERT INTO public.legal_documents (
    ...
    600,
    '2026-08-17T00:00:00-03:00'::timestamptz,
    $privacy_v6$# Política de Privacidade do ItaSuper
...
| Lojistas | CPF/CNPJ, dados do estabelecimento, dados para recebimento e cobrança, cardápio, marca, imagens, planos, valores financeiros, informações de suporte e configurações comerciais. | Operar a loja, cumprir obrigações comerciais, gerar cobranças, relatórios e integrar pagamentos. |
...
$privacy_v6$,
    'Política atualizada com categorias de dados, finalidades, parceiros, IA, retenção, direitos do titular, segurança e transferência internacional.',
    true
  )
  RETURNING id
)
...
FROM (
  SELECT new_terms.id, 'Valores, ofertas e cobranças'::text AS section, 'modified'::text AS change_type,
         'Valores, taxas, prazos e condições comerciais passam a ser os exibidos no checkout, painel ou contrato aplicável; os Termos não fixam valores que podem variar por loja ou plano.'::text AS summary,
         'Decreto nº 7.962/2013, arts. 2º e 4º'::text AS legal_basis, 10 AS display_order FROM new_terms
  UNION ALL
  SELECT new_terms.id, 'Cancelamentos e direitos do consumidor', 'modified',
         'A redação deixa de criar exclusão genérica para alimentos e preserva direitos legais em arrependimento, vício, não entrega e divergência da oferta.',
         'CDC, art. 49; Decreto nº 7.962/2013, art. 5º', 20 FROM new_terms
  UNION ALL
  SELECT new_terms.id, 'Papéis da plataforma, lojas e entregadores', 'modified',
         'Responsabilidades operacionais foram descritas sem afastar direitos legais nem afirmar que os Termos, isoladamente, definem vínculos ou obrigações de terceiros.',
         'CDC; Código Civil; legislação trabalhista aplicável', 30 FROM new_terms
  UNION ALL
  SELECT new_terms.id, 'Recursos de inteligência artificial', 'added',
         'Incluídas regras de transparência, uso responsável e revisão humana para recursos de IA e mecanismos automatizados.',
         'LGPD, art. 20', 40 FROM new_terms
  UNION ALL
  SELECT new_privacy.id, 'Dados, finalidades e parceiros', 'modified',
         'Atualizadas as categorias de dados e os destinatários conforme as integrações efetivamente utilizadas pela plataforma.',
         'LGPD, arts. 6º, 7º e 9º', 10 FROM new_privacy
  UNION ALL
  SELECT new_privacy.id, 'Sales Coach e IA', 'added',
         'Incluída transparência sobre o envio voluntário de conteúdo ao gateway de IA para gerar sugestões comerciais.',
         'LGPD, arts. 6º, 7º e 9º', 20 FROM new_privacy
  UNION ALL
  SELECT new_privacy.id, 'Retenção e exclusão de conta', 'modified',
         'Substituída a promessa de eliminação total em prazo fixo por explicação sobre eliminação, anonimização, registros obrigatórios e ciclos técnicos de backup.',
         'LGPD, arts. 15 e 16', 30 FROM new_privacy
  UNION ALL
  SELECT new_privacy.id, 'Direitos, incidentes e transferências internacionais', 'modified',
         'Corrigidos os prazos de resposta e a descrição de incidentes e transferências para remeter aos mecanismos legais e regulatórios aplicáveis.',
         'LGPD, arts. 18, 19, 33 e 48; Resoluções CD/ANPD nº 15/2024 e nº 19/2024', 40 FROM new_privacy
