...
-- Update handle_new_user to only set trial for paid plans
  ...
  IF _role = 'lojista' AND _store_name IS NOT NULL THEN
    ...
    IF _new_store_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        END,
        true,
        -- Only set trial for paid plans (fixed/hybrid), not for commission_only
        CASE
...
-- Update register_as_lojista to only set trial for paid plans
