...
// Nunca criar pedidos em produção por acidente. Estes testes só devem rodar
// em sandbox explicitamente liberado com RUN_MUTATING_E2E=true.
const RUN_MUTATING_E2E = Deno.env.get("RUN_MUTATING_E2E") === "true";
...
Deno.test({
  name: "cancel-order-refund cancels a temporary sandbox order",
  ignore: !RUN_MUTATING_E2E,
  fn: async () => {
    ...
    try {
      ...
      assertEquals(data?.requires_store_refund, false);
    } finally {
...
Deno.test({
  name: "apply_cancellation_policy works for a sandbox store owner",
  ignore: !RUN_MUTATING_E2E,
  fn: async () => {
    const token = await createOwnerSession();
    const orderId = await createTempOrder();
    try {
      ...
      assertEquals(data?.refund_amount, 0);
      assertEquals(data?.refund_method, "none");
    } finally {
      await deleteTempOrder(orderId);
    }
  },
