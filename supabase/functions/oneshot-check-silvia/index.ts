...
Deno.serve(async (req) => {
  ...
  out.orders_yday_all = await q(`
    SELECT o.id, o.order_number, o.status, o.total, o.commission_amount, o.delivery_fee,
  ...
  `);
  out.plan = await q(`
    SELECT sp.store_id, sp.plan_type, sp.is_active, sp.commission_pct, sp.monthly_fee,
  ...
  out.finalize_stuck = await q(`
    ...
    RETURNING o.id, o.order_number, o.total, o.commission_amount;
  `);
  out.after_orders = await q(`
    SELECT o.id, o.order_number, o.status, o.total, o.commission_amount
    FROM public.orders o JOIN public.stores s ON s.id=o.store_id
    WHERE (s.name ILIKE '%cantinho%silv%' OR s.name ILIKE '%silvia%')
      AND o.created_at >= ${scope} AND o.created_at < ${scopeEnd}
    ORDER BY o.created_at;
