...
`;
  ...
  const pastelao = await q(`SELECT name, slug, pix_direto_enabled, pix_direto_key, pix_direto_key_type, pix_direto_beneficiary FROM stores WHERE slug ILIKE '%pastelao%' OR name ILIKE '%pastel%';`);
