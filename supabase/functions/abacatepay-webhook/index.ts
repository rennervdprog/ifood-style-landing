// Webhook AbacatePay — confirma pagamentos de mensalidade/comissão da plataforma.
// URL: <functions>/abacatepay-webhook?webhookSecret=<ABACATEPAY_WEBHOOK_SECRET>
...
Deno.serve(async (req) => {
  ...
  const data = payload?.data?.pixQrCode || payload?.data?.billing || payload?.data || {};
  const paymentId = String(data?.id || "");
  const externalRef = String(data?.metadata?.externalId || payload?.data?.metadata?.externalId || "");

  console.log(`[abacatepay-webhook] event=${event} id=${paymentId} ref=${externalRef}`);

  if (!paymentId && !externalRef) return json({ ok: true, ignored: "no_identifier" });
  ...
  if (!tx) return json({ ok: true, ignored: "transaction_not_found", payment_id: paymentId });
