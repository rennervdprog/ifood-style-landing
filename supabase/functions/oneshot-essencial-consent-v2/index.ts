...
Deno.serve(async (req) => {
  ...
  out.rpc = await run(`
    ...
    BEGIN
      ...
      END IF;
      SELECT s.id, sp.id, sp.plan_type INTO _store_id, _plan_id, _plan_type
      ...
      IF _response = 'accepted' THEN
        -- Ativa mensalidade cheia imediatamente e reativa loja se estava suspensa por recusa anterior.
        _fee := CASE _plan_type WHEN 'autonomy' THEN 239.90 ELSE 180 END;
        UPDATE public.store_plans
        SET essencial_upgrade_response = 'accepted',
        ...
        WHERE id = _store_id AND status = 'inativo';
        INSERT INTO public.admin_logs(action, metadata)
        VALUES ('essencial_upgrade_accepted',
                jsonb_build_object('store_id', _store_id, 'plan_type', _plan_type, 'new_fee', _fee, 'user_id', _uid));
        RETURN jsonb_build_object('ok', true, 'response', 'accepted', 'new_monthly_fee', _fee);
      ELSE
        -- Recusa: registra e SUSPENDE a loja até o aceite.
        UPDATE public.store_plans
        SET essencial_upgrade_response = 'refused',
            essencial_upgrade_response_at = NOW(),
            updated_at = NOW()
        WHERE id = _plan_id;
        UPDATE public.stores
        ...
        WHERE id = _store_id;
        INSERT INTO public.admin_logs(action, metadata)
        VALUES ('store_suspended_upgrade_refused',
                jsonb_build_object('store_id', _store_id, 'plan_type', _plan_type, 'user_id', _uid));
