...
const orderTabs: { status: OrderStatus; label: string; icon: React.ElementType; color: string }[] = [
  { status: "pendente", label: "Novos", icon: Clock, color: "text-yellow-400" },
  { status: "preparando", label: "Preparando", icon: ChefHat, color: "text-blue-400" },
  { status: "pronto_para_entrega", label: "Pronto", icon: Package, color: "text-purple-400" },
  { status: "saiu_entrega", label: "Saiu", icon: Truck, color: "text-cyan-400" },
...
const AdminDashboard = () => {
  ...
  const todayTotal = orders?.reduce((sum, o) => {
    ...
    return orderDate === today ? sum + Number(o.total_price) : sum;
  }, 0) || 0;
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-card border-r border-border flex flex-col transition-transform duration-300 ${
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={`w-2 h-2 rounded-full ${isOnline ? "bg-green-500" : "bg-destructive"}`} />
        ...
        <div className="p-3 space-y-2 border-b border-border">
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-2.5 text-center">
              <p className="text-lg font-black text-primary">{pendingCount}</p>
            ...
            return (
              ...
              >
                ...
                {item.key === "orders" && pendingCount > 0 && (
                  <span className="ml-auto bg-yellow-400 text-yellow-900 text-[10px] font-black px-1.5 py-0.5 rounded-full animate-pulse">
                    {pendingCount}
                  </span>
                )}
              </button>
        ...
        <div className="p-3 border-t border-border space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className={`flex items-center gap-1 text-xs font-bold ${(onlineDrivers?.length || 0) > 0 ? "text-green-500" : "text-muted-foreground"}`}>
              <span className={`w-2 h-2 rounded-full ${(onlineDrivers?.length || 0) > 0 ? "bg-green-500 animate-pulse" : "bg-muted"}`} />
          ...
          <div className="flex items-center gap-1.5">
            {soundEnabled && (
              <button
                ...
                className={`p-2 rounded-lg border border-border ${soundMuted ? "text-destructive" : "text-green-500"}`}
            ...
            <button
              ...
              className={`p-2 rounded-lg border border-border ${autoPrint ? "text-primary bg-primary/5" : "text-muted-foreground"}`}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-30 bg-card/95 backdrop-blur border-b border-border px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-foreground text-lg">
                {sidebarItems.find(i => i.key === dashboardTab)?.label || "Pedidos"}
              </h2>
          ...
          <div className="flex items-center gap-2">
            {/* Sound prompt (compact) */}
            {showSoundPrompt && !soundEnabled && (
              <button
                onClick={activateSound}
                className="flex items-center gap-1.5 bg-yellow-400/10 border border-yellow-400/30 text-yellow-500 px-3 py-1.5 rounded-lg text-xs font-bold animate-pulse"
              >
                <Bell className="h-3.5 w-3.5" />
                Ativar alertas
              </button>
            )}
            {/* Pending badge in header for mobile */}
            {pendingCount > 0 && dashboardTab !== "orders" && (
              <button
                onClick={() => { setDashboardTab("orders"); setActiveTab("pendente"); }}
                className="flex items-center gap-1.5 bg-yellow-400 text-yellow-900 px-3 py-1.5 rounded-lg text-xs font-bold animate-bounce"
              >
            ...
            <>
                      ...
                      >
                        ...
                        {count > 0 && (
                          <span className={`ml-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-black ${
                            tab.status === "pendente" ? "bg-yellow-400 text-yellow-900 animate-pulse" : isActive ? "bg-primary-foreground/20" : "bg-muted text-muted-foreground"
                          }`}>
                            {count}
                          </span>
                        )}
                    ...
                    return (
                      <div
                        ...
                        className={`bg-card rounded-2xl overflow-hidden border transition-shadow hover:shadow-md ${
                          order.status === "pendente" ? "border-yellow-400/40 shadow-yellow-400/5" : "border-border"
                        }`}
                      >
                        {/* Card Header */}
                        ...
                        <div className="mx-4 mb-2 bg-muted/50 rounded-xl px-3 py-2 space-y-1">
                          ...
                          {order.order_items?.map((item: any) => {
                            ...
                            return (
                              <div key={`obs-${item.id}`} className="pl-6 text-[11px] text-muted-foreground italic">
                                📝 {item.observations}
                              </div>
                            );
                          })}
                        ...
                        {order.status === "pronto_para_entrega" && !order.driver_id && (
                          <div className="mx-4 mb-2 bg-yellow-500/5 border border-yellow-500/20 rounded-xl px-3 py-2.5">
                            <div className="flex items-center gap-1.5 mb-1">
                              <Loader2 className="h-3.5 w-3.5 text-yellow-500 animate-spin" />
                              <span className="text-xs text-yellow-600 dark:text-yellow-400 font-semibold">Aguardando entregador</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className={`w-2 h-2 rounded-full ${(onlineDrivers?.length || 0) > 0 ? "bg-green-500 animate-pulse" : "bg-destructive"}`} />
                        ...
                        {order.driver_id && (order.status === "pronto_para_entrega") && (
                          <div className="mx-4 mb-2 flex items-center gap-1.5 bg-green-500/5 border border-green-500/20 rounded-xl px-3 py-2">
                            <Bike className="h-3.5 w-3.5 text-green-500" />
                            <span className="text-xs text-green-600 dark:text-green-400 font-semibold">🏍️ {getDriverName(order.driver_id)} a caminho da loja</span>
                          </div>
                        )}
                        {order.driver_id && (order.status === "em_transito" || order.status === "saiu_entrega") && (
                          <div className="mx-4 mb-2 flex items-center gap-1.5 bg-cyan-500/5 border border-cyan-500/20 rounded-xl px-3 py-2">
                            <Truck className="h-3.5 w-3.5 text-cyan-500" />
                            <span className="text-xs text-cyan-600 dark:text-cyan-400 font-semibold">🛵 {getDriverName(order.driver_id)} entregando</span>
                          </div>
                        )}
                        ...
                        {["dinheiro", "cartao"].includes(order.payment_method) && (order as any).settlement_code && ["entregue", "finalizado"].includes(order.status) && !(order as any).return_to_store_confirmed && (
                          <div className="mx-4 mb-2 bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-3">
                            {order.driver_id && (
                              <div className="flex items-center gap-2 mb-2 pb-2 border-b border-yellow-500/10">
                                <Bike className="h-4 w-4 text-yellow-500" />
                                <span className="text-sm font-bold text-yellow-600 dark:text-yellow-400">🏍️ {getDriverName(order.driver_id)}</span>
                                <span className="ml-auto text-[10px] bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 px-2 py-0.5 rounded-full font-bold">
                                  #{order.id.slice(0, 8).toUpperCase()}
                                </span>
                              </div>
                            )}
                            <div className="flex items-center justify-between mb-1">
                              <p className="text-[10px] font-bold text-yellow-600 dark:text-yellow-400">🔑 Código de Acerto</p>
                              <button
                            ...
                            <p className="text-3xl font-black text-yellow-600 dark:text-yellow-400 tracking-[0.3em] text-center">{(order as any).settlement_code}</p>
                        ...
                        {["dinheiro", "cartao"].includes(order.payment_method) && (order as any).return_to_store_confirmed && ["entregue", "finalizado"].includes(order.status) && (
                          <div className="mx-4 mb-2 flex items-center gap-1.5 bg-green-500/5 border border-green-500/20 rounded-xl px-3 py-2">
                            ...
                            <span className="text-xs text-green-500 font-bold">Acerto realizado ✅</span>
                          </div>
                        )}
                          ...
                          <div className="mx-4 mb-2 flex flex-wrap gap-1.5">
                            {order.status === "pendente" && (
                              <button
                                ...
                                className="flex items-center gap-1 bg-green-500/10 text-green-600 dark:text-green-400 text-[10px] font-bold px-2 py-1 rounded-lg"
                              >
              ...
              {pendingCount > 0 && activeTab !== "pendente" && (
                <button
                  ...
                  className="fixed bottom-6 right-6 bg-yellow-400 text-yellow-900 font-bold px-4 py-2.5 rounded-xl shadow-lg animate-bounce flex items-center gap-2 text-sm z-30"
                >
