 ...
 * `profiles.delivery_pin` a definir um PIN de 4 dígitos antes de continuar
 * usando qualquer página do app.
 */
const ClientPinChecker = () => {
  const { user } = useAuth();
  ...
  // Não bloquear telas onde o usuário pode estar autenticando/saindo ou em fluxos legais.
  const skipPath =
    pathname.startsWith("/auth") ||
    pathname.startsWith("/portal-parceiro") ||
  ...
  useEffect(() => {
    let active = true;
    if (!user?.id || skipPath) { setNeedsPin(false); return; }
    (async () => {
      ...
      if (role && role !== "cliente") { setNeedsPin(false); return; }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNeedsPin(false);
      setPin(""); setConfirmPin(""); setAck(false);
    } catch (err: any) {
        ...
        <form onSubmit={handleSave} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder="0000"
              className="w-full h-12 px-4 rounded-xl border border-border bg-white text-center text-2xl tracking-[0.5em] font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
            />
          </div>
...
export default ClientPinChecker;