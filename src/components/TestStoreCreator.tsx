...
import { Plus, Store, Eye, Trash2, Loader2 } from "lucide-react";
...
const TestStoreCreator = () => {
  const { user } = useAuth();
  // Produção: criação de lojas-teste desabilitada para evitar bloqueio do Asaas
  // por subcontas com dados fictícios. Reative apenas em ambiente de homologação.
  return (
  ...
  const handleCreate = async (cat: typeof CATEGORIES[0]) => {
    setCreating(true);
    try {
      const { data, error } = await supabase.rpc("admin_create_test_store", {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Crie lojas fictícias para testar o painel de cada categoria sem precisar criar contas de e-mail.
      </p>
