    ...
    'products', coalesce(
      (select jsonb_agg(to_jsonb(p.*))
       ...
       where p.store_id = (select id from s)
         and p.is_available = true), '[]'::jsonb),
    'owner_profile', (
      select jsonb_build_object('id', pr.id, 'full_name', pr.full_name, 'avatar_url', pr.avatar_url)
      from public.profiles pr
      where pr.id = (select owner_id from s)
    ),
...
create index if not exists idx_menu_sections_store_pos
  on public.menu_sections (store_id, position);
