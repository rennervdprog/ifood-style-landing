...
DO 9725 BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user'); END IF; END 9725;
...
DO 9725 BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'financial_transaction_status') THEN CREATE TYPE public.financial_transaction_status AS ENUM ('pending', 'approved', 'paid', 'failed', 'cancelled'); END IF; END 9725;
...
DO 9725 BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'financial_transaction_type') THEN CREATE TYPE public.financial_transaction_type AS ENUM ('commission_charge', 'store_payout', 'driver_payout'); END IF; END 9725;
...
DO 9725 BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'order_status') THEN CREATE TYPE public.order_status AS ENUM ('aguardando_pagamento', 'pendente', 'preparando', 'pronto_para_entrega', 'em_transito', 'entregue', 'saiu_entrega', 'finalizado', 'cancelado'); END IF; END 9725;
