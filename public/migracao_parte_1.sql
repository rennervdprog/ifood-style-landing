...
BEGIN
  ...
  IF NOT _is_physical THEN RETURN NEW; 
  IF COALESCE(NEW.delivery_fee, 0) <= 0 THEN RETURN NEW; 
  IF _delivery_mode != 'own' THEN RETURN NEW; 
  _platform_split := public.get_fixed_plan_platform_split(NEW.store_id);
  IF _platform_split <= 0 THEN RETURN NEW; 
...
BEGIN
  ...
  CASE _order.status
    ...
    WHEN 'preparando' THEN
      ...
      ELSE _fee_percent := 20; 
    WHEN 'pronto_para_entrega' THEN
      IF _minutes_in_status >= 20 THEN _fee_percent := 0;
      ELSE _fee_percent := 40; 
    WHEN 'saiu_entrega' THEN
      IF _minutes_in_status >= 20 THEN _fee_percent := 0;
      ELSE _fee_percent := 60; 
